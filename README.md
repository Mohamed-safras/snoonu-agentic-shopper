...
- **Real catalog, real money, real orders** — live MCP search, genuine QAR prices, and a working 45-minute pay link from `create_order`, validated end-to-end (no broken pay-link cards — Snoonu errors surface as real messages, not a generic failure).
...
- **Multilingual, end-to-end** — English, Arabic, Sinhala, and Tamil. Product Q&A and compare answers respond natively in whatever language you *ask* in, even if the UI toggle is set to English. Replies are machine-translated from a single English source (MT-first), so content stays consistent across languages — only the wording changes.
...
- **Delivery & tracking maps** — live route + ETA from Snoonu HQ to the destination city (Leaflet + OSRM), draggable pin for an exact drop-off.
...
A genuine **orchestrator + specialist multi-agent system** wired to a self-hosted mock **Snoonu MCP server** (Streamable HTTP, no auth — see [`snoonu-mcp-server`](https://github.com/) for the server itself).
...
- **Prices are the mock Snoonu MCP's amounts** (QAR); nothing is converted or invented. This is a self-hosted **mock** catalog (Qatar-flavored synthetic + Open Food Facts grocery data), not a live integration with Snoonu's real systems.
...
3. Start the mock MCP server (separate project) and point `SNOONU_MCP_URL` at it in `.env.local` (defaults to `http://localhost:8000/mcp`).
4. Run: `pnpm dev` → http://localhost:3000

## Verify it works

- `GET /api/debug/mcp` (dev only) — exercises all 7 MCP tools live.
- In the chat: _"flowers for my wife"_ → real shelf → add to cart → _"deliver to Doha"_ → pick a date → fill details → **Place order** → open the pay link → track with the order number.
- Switch language (top-right) and try a turn in Arabic / Sinhala — also ask a product or compare question without switching the toggle and confirm it answers natively.
...
`vercel` (or import the repo). Set the env vars from `.env.example` in the Vercel dashboard, including `SNOONU_MCP_URL` pointing at your deployed mock MCP server. The chat route runs on the Node runtime with streaming.
...
_A self-hosted mock Snoonu shopping concierge · real catalog (mock data) · no sign-up · Qatar-wide 🇶🇦_
